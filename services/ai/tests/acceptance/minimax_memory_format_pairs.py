...
def compare(source: Path, output: Path):
    ...
    for i, candidate in enumerate([False, True, True, False]):
        def sections():
            ...
            return {key: value.replace('不要 markdown', '不要在JSON对象外使用Markdown') +
                    (SHAPE if key in {'system', 'tool_followup', 'recovery'} else '')
        ...
        row.update(format_candidate=candidate, experimental_shape_instruction=SHAPE if candidate else None,
...
if __name__=='__main__':
    ...
    args=p.parse_args();compare(args.source.resolve(),args.output.resolve())
