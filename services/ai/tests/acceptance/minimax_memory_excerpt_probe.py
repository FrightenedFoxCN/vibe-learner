...
def run(source, output, repetitions):
    ...
    with (output / 'report.jsonl').open('x') as stream:
        for repetition in range(repetitions):
            variants = ('head180', 'head_tail180', 'complete1600')
            if repetition % 2:
            ...
            for variant in variants:
                ...
                with patch.object(study_memory, '_truncate', excerpt), patch.object(study_memory, '_build_candidates', build), patch.object(ProviderRequestAdapter, 'request_chat_completion', observe), TestClient(app) as client:
                    ...
                    row = {'scope': 'live_study_operation_receipt_readback', 'model': 'MiniMax-M3', 'git_revision': revision,
                        ...
                        'trace_limitation': 'Experimental excerpt construction and fixed seed-session selection; production domain admission/commit. Embedding requests not instrumented.'}
...
if __name__ == '__main__':
    ...
    run(args.source.resolve(), args.output.resolve(), args.repetitions)
