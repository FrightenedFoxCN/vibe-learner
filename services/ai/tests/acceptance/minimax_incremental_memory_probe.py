...
STATUS_MEANING = ('active表示当前仍有效；revoked表示记录明确撤销且没有新有效约定；'
                  'unknown仅表示没有足够记录判断状态。值写未知不代表状态必须unknown。')


class SourcedFact(Strict):
    value: str
    status: Literal['active', 'revoked', 'unknown'] = Field(description=STATUS_MEANING)
    value_source_id: str = Field(description='最初提供当前value的用户记录编号；引用归档不改变来源。')
    status_source_id: str = Field(description='实际设立、变更或撤销当前状态的用户记录编号；再次确认或归档不替换事件来源。')


class SourcedState(Strict):
    meeting: SourcedFact
    code: SourcedFact
    preference: SourcedFact


class StatusAnswer(Answer):
    code_status: Literal['active', 'revoked', 'unknown'] = Field(description=STATUS_MEANING)


def segments(rep, renewed=False):
    updates = [
    ...
    if renewed:
        updates.append(f'[u4] 新约定：现在启用新暗号红风筝{rep+51}，此前旧暗号仍作废。碰头地点、称呼和关系不变。')
...
def run(output, repetitions, schema_candidate=False, state_semantics=False, renewed=False):
    ...
    with output.open('x') as stream:
        def save(row):
            row.update(prompt_variant='explicit-json-schema-v1' if schema_candidate else 'baseline',git_revision=revision,scope='synthetic_incremental_compression',fixture_version='updates-revocation-renewal-v2' if renewed else 'updates-revocation-address-v1',state_semantics_candidate=state_semantics,
        ...
        for rep in range(repetitions):
            chunks=segments(rep, renewed); expected={'meeting':f'北门第{rep+9}号长椅','code':'未知','code_status':'revoked','address':'林舟','relationship':'平等同行'}
            if renewed:
                expected.update(code=f'红风筝{rep+51}',code_status='active')
            answer_contract=StatusAnswer if state_semantics else Answer
            ...
            for variant in (['prose','state'] if rep%2==0 else ['state','prose']):
                ...
                for stage,chunk in enumerate(chunks):
                    contract=Prose if variant=='prose' else SourcedState if state_semantics else State
                    ...
                    if state_semantics:
                        instruction += STATUS_MEANING
                        if variant=='state':
                            instruction=instruction.replace('source_id。source_id引用输入u1/u2/u3；', 'value_source_id、status_source_id。引用输入u1/u2/u3/u4；')
                    ...
                    row,_=call([{'role':'system','content':'依据提供资料回答，不推测缺失约定。'} ,{'role':'user','content':json.dumps(memory,ensure_ascii=False)},question],answer_contract)
                    row.update(repetition=rep,variant=variant,stage='answer',expected=expected,facts_correct=row.get('reply')==expected);save(row)
            row,_=call([{'role':'system','content':'依据提供资料回答，不推测缺失约定。'}]+[m for c in chunks for m in c]+[question],answer_contract)
...
if __name__=='__main__':
    ...
    p.add_argument('--state-semantics',action='store_true')
    p.add_argument('--renewed',action='store_true')
    args=p.parse_args()
    if not 1<=args.repetitions<=10:p.error('repetitions must be 1..10')
    run(args.output.resolve(),args.repetitions,args.schema_candidate,args.state_semantics,args.renewed)
