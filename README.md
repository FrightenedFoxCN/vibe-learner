...
- 设置页的“验证模型与当前功能”只在会话内探测 `/models` 与当前模型的最小代表请求，不持久化探测密钥或结果；真正持久化仍依赖设置字段写回（例如选择模型、回填能力开关）。模型已列出、鉴权通过和当前 feature 可调用是分层结论。
...
- `docs/harness/README.md`：按用户工作流拆分的 Harness 工程手册、数据库映射与配置预算
