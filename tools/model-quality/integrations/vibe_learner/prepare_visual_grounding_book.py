    ...
    for review_path in a.review:
        ...
        for page in review['candidates']:
            ...
            for task in selected:
                ...
                source={'detector':'book','path':str(dest.resolve()),'sha256':page['png_sha256'],'original_pdf_path':str(pdf),'original_pdf_sha256':review['pdf_sha256'],'official_source_url':review.get('official_source_url'),'physical_page':page['physical_page'],'printed_page':page.get('printed_page'),'render_scale':page.get('render_scale'),'image_size':[width,height],'query':query,'include_proposal_table':True,'privacy':'user-provided private document; public export forbidden','transforms':'exact reviewed full-page PNG; no crop/downscale/quantization'}
                annotation_scope=review.get('review_scope') or review.get('annotation_method')
                if not annotation_scope:raise ValueError('annotation_scope_required')
                object_class=page.get('type') or task['id']
                gold={'expected':'found','target_px':box,'width':width,'height':height,'object_class':object_class,'minimum_iou':.5,'minimum_coverage':.8,'tolerance_px':task.get('tolerance_px'),'annotation_scope':annotation_scope}
