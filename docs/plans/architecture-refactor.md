...
状态：七项重构已实施并通过阶段门禁（2026-09-10）；已登记为 0.3.1，桌面安装包发布结果以远端发布流水线为准。本文件维护七项重构的明细，根 [TODO](../../TODO.md) 作为统一索引。与 [统一 Logging / Debug](unified-debug.md) 共享基础边界，不要求先做完全仓重构。
...
- [x] Persona 草稿、已保存基线、选中身份和未保存离页保护归入 `usePersonaDraft`。晚到初始列表只初始化未操作过的草稿；已保存旧快照仅更新基线，不替换后续编辑。5 项真实 Hook 测试覆盖初始化竞争、切换确认、baseline 与链接/beforeunload 清理。名称输入补齐关联 label；真实浏览器验证拒绝离页保留输入及接受后导航。Web 类型/可靠性/生产构建及 17 项 Chromium 验收通过。
...
- [x] Persona 保存/重载/删除应用流程归入 `usePersonaPersistence`，草稿 epoch 区分继续编辑与替换新草稿，保存旧快照只为同一草稿推进基线。删除期间的新编辑保留为未保存草稿，切换后的草稿不受旧删除影响；重复保存被入口守卫合并为单次写。7 项并发/卸载测试保留 PATCH/delete revision 证据，生产浏览器验证创建在途改名及读回绑定；Web 类型/可靠性/生产构建和 18 项 Chromium 验收通过。
...
- [x] `useStudyMessages` 持有 learner 发送入口和自动请求身份，使用显式 send/query port 与恢复控制器协作。Session lookup 期间即合并重复发送，结果不明或需刷新时拒绝新 POST；保留 revision、附件、隐藏答题前缀和 truthful admission/uncertain 区分。6 项真实发送/恢复 Hook 测试、Web 类型/可靠性/生产构建及 21 项 Chromium 回归通过。
