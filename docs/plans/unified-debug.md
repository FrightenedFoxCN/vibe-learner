...
2026-09-10 Tavern 浏览器取消验收：真实取消控件创建新 action 并继承进行中 flow，
其读回请求保持关联。原生成请求 409，Room 只留用户消息，持久化诊断无正文或
演员保存引用。13 项 Chromium 场景通过；测试 provider 依据真实租约检查结束
等待，不声称能强制终止上游请求。OCR/原生成功路径、性能及完整验收继续。

2026-09-10 真实 OCR 成功验收：本机缓存 ONNXTR 模型处理合成纯图像 PDF，实际
识别标题并提交 Document；诊断关联真实六阶段 trace，排除文件名和内容。单页
功能样本 3.020 秒，模型指纹和结果已归档至 docs/acceptance；不作为质量或延迟
门。原生成功路径、开销及最终完整验收继续。

2026-09-10 原生验收发现并修复采集启动缺口：建表配额锁短暂竞争现在和连接配置
共同重试，关闭时正确停止并记账。33 项专项测试、完整 release（745 后端测试、
Harness/shared/Web 与生产构建）通过；原生重启已实际写入事件。设置页当前
Debug 摘要仍发现原始凭据字段暴露，正作为下一项修复，不关闭总体验收。

2026-09-10 设置页 Debug 修复及原生导出验收：原始设置、探测缓存和错误改为
明确白名单摘要；10 项设置测试、完整 check、生产构建及 14 项 Chromium 场景
通过。原生解锁已有测试 Vault 后复查摘要，通过 macOS 保存对话框导出并严格
读回 122 条事件，实际合成密钥不在包中。证据见验收记录；原生创建/保存/锁定
诊断链、暂存失败计数调查、其余开销与总体验收继续。

2026-09-10 渲染与原生开销检查点：实际 React 时间线及解码器在 500 条上限下
通过预设本机 P95 预算；原生暂存普通写入与锁竞争通过，但满队列轮转 P95
25.21ms 略超 25ms，失败原始样本已归档，不关闭性能验收。11 项普通 Rust
测试通过；详见 docs/performance/diagnostic-render-and-spool-v1.md。

2026-09-10 逐项审计更新：已重写验收表顶部，将 14 项浏览器场景、真实 OCR、
原生保存/锁定/导出、恢复与量测结果归到原始要求；本次 110 项后端诊断测试
全部通过。新确认导入动作只记录 JSON 读取，后续领域校验和过期结果拒绝缺少
完整动作结论，列为下一实现项。原生创建/清空、完整流程开销、存储例外与
最终总体验收继续，见 unified-debug-acceptance.md。

2026-09-10 导入完整动作补齐：Persona/Scene 导入包住读取、原领域校验和草稿
应用围栏；读取子动作与父动作共享关联，格式拒绝为失败，过期结果为取消，
成功只表示草稿已应用。Python/TypeScript 和查询/导出 schema 同步。3 项专项、
23 项后端、完整 check、生产构建及 16 项 Chromium 场景通过；真实文件导入
无正文/文件名泄露或误保存请求。原生创建/清空、完整开销与最终验收继续。

2026-09-10 Persona 完整后端请求链开销：30 对诊断开/关样本均实际生成候选、
保存其内容并核对持久化读回，保留相同 canonical Harness。配对开销 P95
18.88ms，低于预设 50ms，最大差值 64.77ms 保留。此为本机 mock 后端链量测，
其余业务流程、原生及满暂存队列问题不由此关闭；原始报告已归档。

2026-09-10 Scene 完整后端请求链开销：30 对样本均完成生成、保存整棵场景树、
...
2026-09-10 Scene 开销归因：分请求及 GC 观测发现新一轮四个慢样本均在生成阶段
重叠 46–62ms 全代 GC，诊断开/关两侧都有。每个样本独立进程、保留默认 GC
后 30 对 P95 增量 6.23ms、最大 8.28ms，通过原定门槛。原始失败与两组新证据
均归档，不修改运行时 GC、不声称长时 GC 保证；其余流程与原生性能仍待验收。

2026-09-10 Document→Planning 完整开销：独立进程 30 对实际上传、解析流、Document
读回、Planning 流及 Plan 读回，完整提交投影与持久化一致；4 个真实 operation、
7 条终态 trace、25 条关联事件，零丢弃/读写失败。P95 增量 10.92ms、最大
12.12ms，通过原定 50ms；单页文本 PDF/mock 计划范围，Study/Tavern 与原生继续。

2026-09-10 Tavern 完整开销：单人和三人顺序生成分别 30 对独立进程样本，覆盖
创建 Room、运行轮次、消息及 Run 读回；全量投影、演员顺序、消息序号与 canonical
提交核对通过。P95 增量分别 5.74ms/10.26ms，均低于 50ms，零丢弃/读写失败。
原始样本已归档；Study 与剩余原生性能继续。

2026-09-10 Study 完整开销：30 对独立进程样本覆盖创建 Session、聊天、操作回执
及 Session 读回；修订/Turn 序号只增一次，回复/引用/Character Events 与完整回执
一致，关联一条真实提交 trace。P95 增量 22.07ms、最大 24.87ms，通过 50ms，
零丢弃/读写失败。原生/渲染性能及最终验收继续。

2026-09-10 原生同步并行实验：测试环境保留三次目录屏障及计数先发布顺序，
30 对两个 pending 文件同步对照；并行 P95 18.61ms、顺序 17.88ms，尾部无收益，
不接入运行时。原始满载 25.21ms 失败仍保留，未放宽预算或削弱持久化。

2026-09-10 嵌套事件渲染：实际 React/严格解码器在五类嵌套字段、4096 字符标签、
...
2026-09-10 原生 WebView 开销完成：隔离 Tauri 可见窗口复用 Chromium 同一量测
函数和实际 React/严格解码器，普通/复杂事件各 30 样本累计 500 条均通过；复杂
P95 首屏/追加/展开/卸载 51/51/33/35ms。不访问 Vault/业务数据，未改生产运行时。
原生满载 I/O 超标及最终逐项关闭审计仍开放。

2026-09-10 原生优化构建复测：同一实际 spool 探针在 --release 下满载 P95
25.06ms，仍超过 25ms；普通与竞争通过。失败完整归档，不四舍五入为通过，
不修改开发编译策略。下一步计数检查点方案需先证明崩溃/撕裂/迁移安全性。

2026-09-10 双槽检查点原型：仅测试编译，版本化定长记录及 SHA-256 校验保留另一
有效槽，逐字节撕裂/损坏、截断及真实子进程中途退出恢复测试通过。30 对检查点
P95 从 8.41ms 到 4.12ms；生产写入器未改变，须先完成真实旧格式/pending 迁移、
旧版拒绝新格式及完整 spool 验收后采用。

2026-09-10 原生计数检查点接入：双槽持久化保留原进程锁、删除屏障及事件发布顺序，
旧 count/pending 可恢复迁移，旧二进制遇新格式拒绝日志写入而非使用过时计数。
