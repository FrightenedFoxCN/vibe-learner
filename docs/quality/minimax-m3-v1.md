...
[两个新Planning操作](evidence/minimax-macbeth-native-image-v1.jsonl)固定同源/严谨人格，取消实验图像注入、保留完整六工具目录，仅首轮指定read_page_range_images。两例都提交；[审查](evidence/minimax-macbeth-native-image-review-v1.json)确认第一例实际调用读图，后续SDK请求含两张800×1222页图，对应生产144 DPI。第二例首轮只调用detail/content，整条链没有图片，不能算作原生视觉成功样本。
...
[回归与实测证据](evidence/minimax-planning-geometry-repair-review-v1.json)：旧实现3项测试共6个失败（含子用例），反映规则不齐、未触发修复；修复后完整后端834项通过。补充同页章节正例后，19项定向测试再次通过；完整套件在该正例加入前运行，未冒称已跑835项。

[两次真实M3修复](evidence/minimax-planning-geometry-repair-live-v1.jsonl)在真实Planning admission/commit/readback路径中，明确注入一个schema合法但章节倒序或片段倒序的初始提案，初始生成不是M3调用。两例各只调用M3一次完成无工具修复，均已提交，顺序均纠正；持续失败的确定性测试仍确认只允许一次修复。
...
[四次实际M3修复](evidence/minimax-repair-timing-clarification-v1.jsonl)按原文—候选—候选—原文运行，初始提案完全相同，均各一次真实调用并提交。[审查](evidence/minimax-repair-timing-clarification-review-v1.json)候选2/2明确分配阶段且合计30分钟（2+5+4+5+8+6、5+10+10+5），原文1/2（3+8+7+7+5），另一例只说三段合计30，没有阶段时长。
...
[15/45分钟四例迁移](evidence/minimax-repair-timing-transfer-v2.jsonl)均各一次真实修复并提交。[审查](evidence/minimax-repair-timing-transfer-review-v2.json)15分钟候选3+4+3+3+2满足要求，原文没有分配；45分钟两组都只保留泛化任务，没有落实时间。结合轮次79，候选3/4、原文1/4通过该时长指标，但样本小、初始提案为故障注入，不能推广为自然任务通过率。15分钟候选还把p.2引文归到p.1并保留OCR碎片，不能把时长通过当成内容通过。
...
[两次生产措辞复测](evidence/minimax-repair-timestamp-production-v1.jsonl)未覆写修复提示，只注入已标明的章节/片段倒序初始提案。[复核](evidence/minimax-repair-timestamp-production-review-v1.json)各一次M3调用后提交，8+10+12和5+10+10+5均合计30。首例对sisters与叙事者的表述仍不够准确，故仅确认时间戳规则不再与活动时长直接冲突。19项定向、完整后端835项通过。
...
沿用原开放式周末建议题，再加入[预设预算标准](evidence/minimax-tavern-budget-rubric-v1.json)：冲突题140分钟/60元超出120分钟/50元，可行题100分钟/40元满足限制；出行时间已含乘车，不重复计算。两次重复均覆盖direct与facilitated，直接模式轮换人格，多人模式按真实调度执行。

[12次运行](evidence/minimax-tavern-budget-sweep-v1.jsonl)共25次SDK调用。[复核](evidence/minimax-tavern-budget-sweep-review-v1.json)：开放式4/4运行完整提交，6条角色回复都恰好两条列表；但未知城市/季节仍给“下午四点”“上午十点”最适合侧光等缺条件判断，额外饮品/胶片消费也未量化，不能证明真实价格或确切超支。未重现旧样本的明确140分钟开放式路线。
...
先保留[错误探针批次](evidence/minimax-tavern-thinking-probe-failure-v2.json)：thinking误放LiteLLM顶层，12次运行在毫秒级适配错误后失败，无usage或回复；旧观测未保存底层具体错误，因此不把24次适配尝试记为已到达模型的调用。改用官方extra_body路径后，通过HTTPX核验序列化thinking.type和max_tokens，并补充无内容错误分类观测。

[正确重跑的12次运行](evidence/minimax-tavern-budget-profile-v3.jsonl)使用相同冲突题，比较adaptive800、adaptive4096、disabled800，各direct/facilitated两次。[复核](evidence/minimax-tavern-budget-profile-review-v3.json)：adaptive800仍0/4完整提交，8次请求全部length；其他两组均4/4完整提交，各6条角色回复，无修复调用。所有实际请求的thinking和预算与配置一致。
...
固定4096/adaptive和冲突题，按基线—格式—可行条件—可行条件—格式—基线执行，每个cell各direct/facilitated一次。[12次真实运行](evidence/minimax-tavern-constraint-reminders-v1.jsonl)均提交，18次SDK调用均stop，无修复。提醒在实验探针的SDK payload层追加，明确记录原文与作用位置；生产preflight/注册prompt未更改，终态trace不是实验提示已注册的证明。

[逐条审查](evidence/minimax-tavern-constraint-reminders-review-v1.json)：新基线6/6满足两条列表，格式提醒也是6/6，可行条件提醒5/6。因此此前adaptive4096的2/6不能作为本轮格式收益的固定基线。明确指出真实线路/价格需要核实的回复分别1/6、2/6、6/6；可行条件提醒有方向性收益，但首个direct仍列四项，另一条留下“10分钟（应为10元）”的已纠正单位笔误，还有约35–40元的模糊分配。全部原始总数算对，不将这些缺陷混为原始算术错误。
...
[论文摘要记录](evidence/minimax-tavern-self-correction-references-v1.json)包括Reflexion（任务反馈与反思记忆）和Large Language Models Cannot Self-Correct Reasoning Yet（缺外部反馈时自我修正可能退步）。只读公开摘要，不声称复现其方法或取得同等效果。这里用外部题目标准分别审查格式、算术、条件与人格，不把“要求检查”当作正确性证明。暂不采用生产提醒，下一轮需换题/换格式检验迁移，或用有依据的外部反馈纠正已观察到的问题。
...
[8次运行](evidence/minimax-tavern-grounding-transfer-v1.jsonl)都提交，共13次SDK调用，无length；首个基线facilitated的初始回复非严格JSON，经一次恢复后提交。[逐条复核](evidence/minimax-tavern-grounding-transfer-review-v1.json)基线3/6满足三条列表且无附加段落，提醒组6/6；基线三条把取舍另起为列表外段落。明确询问三个缺失条件分别5/6、6/6，但本题用户已要求处理未知条件，不是无提示的泛化证据。
...
复用Macbeth两页完成快照，统一姓名、关系、摘要、系统指令与语气，仅替换一个方法槽的content：引文/语境/依据检查，对比例子/不同解释/适用边界。再交叉文字与文字加第2页100DPI图，使用相同五工具目录。[标准](evidence/minimax-planning-method-only-rubric-v1.json)在阅读本批输出前写入；[上下文核验](evidence/minimax-planning-method-only-context-v1.json)四组去除persona后相同，实际人格配置除槽content外也完全一致。

[四次操作](evidence/minimax-planning-method-only-v1.jsonl)共19次调用，3份提交。[审查](evidence/minimax-planning-method-only-review-v1.json)引文方法两份以引文核对与依据检查组织任务；探索文字组比较姐妹能力和glass解释边界，显示方法槽会影响活动。三份阶段分别8+10+7+5、2+18+8+2、2+8+6+4+5+3+2，均30分钟，仍不等于内容正确。
...
在相同两页源快照上注入schema合法但章节倒序或片段倒序的初始提案，每种故障按现行—候选—候选—现行各一次，且逐组验证初始提案完全相同。[8次真实修复](evidence/minimax-geometry-hint-pairs-v1.jsonl)均仅一次M3调用；初始提案为合成，不计自然生成。候选说明章节起始页和章内片段起始页非递减、允许同页，活动回看顺序放focus/today_tasks；同时加入保留章节/片段/标题/页码的约束。

[审查](evidence/minimax-geometry-hint-pairs-review-v1.json)：候选4/4提交，现行3/4。现行首条在修复中引入duplicate_schedule_unit_ref，并非继续倒序；因此不能把差异完全归因于顺序说明。候选3/4明确30分钟，现行2/4；两组各有一条只机械修复结构、保留无时长的泛化活动。小样本合成故障不是自然修复通过率。
...
去掉冻结章节/标题/范围的句子，保留章节和片段非递减、同页允许重复、活动顺序放focus/today_tasks的说明。[8次单次真实修复](evidence/minimax-geometry-concise-pairs-v1.jsonl)仍按两类故障各现行—候选—候选—现行，初始合成提案在故障组内一致。

[复核](evidence/minimax-geometry-concise-pairs-review-v1.json)：两组均4/4提交、3/4明确分配30分钟，全部保留两页来源覆盖，没有观测到提交或时长收益。候选与基线各一条保留泛化活动；基线另把正文页称为Front Matter，及声称六阶段各对应章节但只提供三章。候选仍有将旗帜列入武器类别等语义问题。此前进度中提到的“未扩展到第8页之后”未明确指PDF或印刷页，只能记为页码表述歧义，不能认定已证明页码混用。
...
[4次真实操作](evidence/minimax-natural-order-capture-v1.jsonl)均提交，总33次调用，6次工具拒绝。[核验](evidence/minimax-natural-order-capture-review-v1.json)本批无目标修复，进程正常结束且无私有请求快照或重放结果，所以重放调用数为0，未测试候选自然修复效果。不能因这次没有重现而抹去轮次85失败。重放分支仅通过编译，尚未实测，明确保留此限制。
...
真实任务要求先看p.2图5分钟、回p.1核对10分钟、回p.2读glass限定10分钟、对照解释5分钟。[首个自然操作](evidence/minimax-natural-backtrack-source-v1.jsonl)即触发not_ordered：初始章节起始页2、1、2、1–2，实际把活动阶段映射成了章节顺序。原有一次修复后转为content_slices outside_chapter，3次模型调用后未提交。捕获完整请求与工具后Study Units，私有文件权限实际为0600，仅在临时目录保留。

[四次定点重放](evidence/minimax-natural-backtrack-replay-v1.jsonl)初始提案、工具后上下文和图片相同，只在候选追加简洁排序反馈。[审查](evidence/minimax-natural-backtrack-review-v1.json)基线2/2校验通过，却将全部四章都扩为1–2页，来源定位变粗；候选1/2通过，按1、2、2、2排序并在focus保留实际活动顺序2、1、3、4，但把最终跨页对照定位缩成p.2，还将schedule_chapters等实现词写进用户活动说明。另一候选仍倒序。
...
检查重复估时发现确定性问题：estimate_plan_completion对不超过两个Study Units固定增加0.08跨度扣分，并据此返回coarse_segmentation与“把过宽的学习单元拆细”。[复现与修复记录](evidence/minimax-short-source-estimate-review-v1.json)一份仅两页且有两个子标题的单元得到84分、“可以收束成稿”，同时又被要求拆细；单元数量不能证明实际跨度过宽。
...
[两次修复后真实Planning](evidence/minimax-short-source-estimate-live-v1.jsonl)均提交，共17次调用。5次实际估分全部不再把两页材料标成过宽；但都因子标题元数据为空停在42分，继续要求读详情/页范围。第一例14次调用仍出现修订参数、重叠和操作配额拒绝，第二例3次，不能声称减少调用或提升稳定延迟。现有估分只看源结构元数据，并未评审实际提案，重复估时不能凭活动草案的完善自动变高，属于后续需单独检验的问题。
...
[四次真实操作](evidence/minimax-estimate-scope-pairs-v1.jsonl)按基线—候选—候选—基线，都已提交。[审查](evidence/minimax-estimate-scope-pairs-review-v1.json)每条恰好估分一次，无估分次数收益；总调用基线6、候选9，候选首条出现一次重叠修订拒绝。不能把范围说明当成已证实的效率优化。
...
[四组实测](evidence/minimax-french-estimate-description-v1.jsonl)按基线—候选—候选—基线，SDK尝试5/4/6/17次，含usage的上游响应5/3/5/16次；仅首组提交，其余触及wall-time预算。工具拒绝5/4/3/31次，估分调用1/0/1/1次，未证明候选收益，不采用生产描述。缓存有前缀复用，但仍发生重复工具拒绝和时间耗尽，不能据命中推导整体性能合格。
...
修正配置后，[两组法文长书实测](evidence/minimax-french-native-images-v1.jsonl)均触及wall-time预算，未生成可审查计划。文字组4次SDK尝试、3次带usage响应、2次同轮配额拒绝；图片组7次尝试、6次带usage响应、2次同轮和2次操作配额拒绝。失败不算候选成功，无法给最终事实、时长或人格活动评分。

[核验](evidence/minimax-french-native-images-review-v1.json)文字组确实只提供五工具，图片组提供六工具并两次请求read_page_range_images；图片组各次请求图像part数0/1/1/1/4/4/4。总和15包含重复携带和最后一次失败尝试，不能说读了15个不同页面，也不能确认失败请求已被上游处理。此次原始脚本未捕获图片页码，仅能证明图片已进入实际请求，不能证明证据选页正确。
...
从原书PDF31–33提取三页正文，独立解析后运行60分钟任务。[实测](evidence/minimax-french-short-native-v1.jsonl)5次SDK调用后提交，图像part序列0/3/3/3/3，确实使用页图。此诊断与长书的两周目标不同，不据此断言长上下文是唯一原因。

[源图审查](evidence/minimax-french-short-native-review-v1.json)保留了“每点层芽为局部环”和一般几何空间并非全为仿射概形，比较例子与适用边界也进入活动；但today_tasks把层同态写成f_*(O_X)→O_Y，原文1.4为O_Y→f_*(O_X)，方向反了。focus两段各30分钟，中间与收尾另提5分钟但未明确包含关系，记录为时长歧义，不武断累加成70分钟。

原文页3有手写伴随注记，且生成的Hom_{Esg°}(Spec A,X)等价于印刷定理X→Spec A，不能把反范畴记法误报成另一次箭头错误。独立于本例，已在阅读配对输出前写好[同源四组审查标准](evidence/minimax-french-short-native-rubric-v1.json)，覆盖定义、公式、例子、注记来源、时间、人格与实际取证。四组仍在运行，未声明图片质量收益；维护者评审不构成独立认证。

## 轮次 97：法文原生图像四组提交差异，不等于完整质量收益

[文字—图片—图片—文字四组](evidence/minimax-french-short-native-pairs-v1.jsonl)共29次SDK调用且都有usage，分别7/6/7/9次。文字1/2提交，图片2/2提交；首个文字组在最终回复到达后仍触及领域wall-time预算，未计成功。工具拒绝3/1/2/3次。两图片组都实际读取PDF1–3文字与三张页图；首个文字组只成功读第1页，另两次同轮读取被拒；末个文字组成功读1–3页。

按预先建立的标准[逐组审查](evidence/minimax-french-short-native-pairs-review-v1.json)，三份提交计划都未完整通过。图片首组层同态方向正确，却把一般开子空间与X_s特殊开集写成等价，任务分钟8+5+5+7+8+4+8+5仅50，与概览60不符。图片第二组任务总60，但按页分组15/25/20与排期标题15/20/25不一致；还使用germe/valeur“等价”措辞，并把unit_llm内部ID和schedule术语泄漏到用户文案。
...
[省略—none—none—省略四组](evidence/minimax-french-tool-policy-pairs-v1.jsonl)统一在三轮工具响应后触发相同成稿提醒，共17次调用，全部提交。HTTP观测与17次SDK请求对应：none成稿时保留六工具并发送tool_choice=none，省略组无目录；本批触发后均未再返回工具调用。运行时授权未改，不能把SDK控制当作硬准入上限。

[内容审查](evidence/minimax-french-tool-policy-pairs-review-v1.json)四例都未完整通过。省略首例20+20+20满足时长，却混用层映射与拓扑逆像、写反投影芽映射，还误导读者不能默认纤维积存在，原文1.8实际说明小投射极限存在而不使用。none首例没有分配各阶段分钟且误用f#做函数复合；none次例用f_x(m_x)替代应为m_y的源极大理想，A–F合计54分钟，另6分钟又标作已嵌入，分配关系不清。末省略例只明确20分钟，并将p.3的1.7–1.8定位到p.2。
...
[安全指标与源图复核](evidence/minimax-native-vision-ocr-review-v1.json)：macOS26.6.2、Vision请求revision3，两次56行且行文字一致，框架调用995/729ms（不含Swift编译与启动）。识别642个中文字符，保留“时期性”、1930、1992、20世纪40年代晚期、微粒及dreadfully。本页输出先完整左页、后右页，只跨列一次，译文续接方向正确。
...
[确定性反例](evidence/minimax-estimator-capability-before-v1.json)在没有计划活动输入的情况下，对空focus、“核对30分钟活动”和“目标尚未核对，活动还没有编写”均返回92分及“可以收束成稿”，空focus还声称行动性已经足够。实现实际上只检查Study Unit页跨度与子标题数量，focus只是回显。

修正初始提示，不再要求该工具检查目标覆盖和行动可执行性；高分标签、默认建议与trace摘要改为描述结构元数据，并要求另行核对原文、目标、活动与时长。[同一反例修正后](evidence/minimax-estimator-capability-after-v1.json)分数不变，能力表述不再宣称计划活动已经通过。没有更改分数公式、结果schema、工具目录或调用上限。20项定向回归通过。

[真实M3核验](evidence/minimax-estimator-scope-live-v1.jsonl)两次调用后提交，先读取详情和页范围，之后直接生成，**没有遵循输出前调用估分工具的指令**；因此它验证新初始提示可生成并提交，不构成新工具文案已被模型读取的证据。计划按5+8+10+4+3分配30分钟，包含复盘，并把探索方法关联到词语比较与解释边界；正文评论仍被泛称为注释。没有把一次少调用当作性能优化，也不认为指令遵循已合格。
...
[两次实测](evidence/minimax-estimator-exposure-v1.jsonl)分别3/4次调用并提交；后续请求观测到实际工具结果，均为42分、“建议继续细化学习单元结构”，并列出子标题和页范围证据缺项。这补充轮次106未调用估分的证据缺口，只覆盖低分标签，不证明高分文案的模型效果。
...
据此新增[52条合成历史对照](evidence/minimax-temporal-compression-v1.jsonl)，包含两组人物的不同见面约定，区分消息时刻、原约、改约后的见面时间和归档时刻。基线/显式日期保留两种摘要以ABBA顺序各执行两次，另各一次完整历史回答；摘要预算450字，输出2048token。是直接provider诊断，没有领域Harness准入或生产压缩采用。
...
仅将输出预算调至4096，保持历史、摘要指令和ABBA顺序不变。[10次调用](evidence/minimax-temporal-compression-4096-v2.jsonl)均以stop结束；两份候选摘要及五项回答全通过，完整历史两次也全对。基线一份回答JSONDecodeError，另一份只答对三项见面时间，改约和归档时刻仍缺失。新增可配置预算与显式参数记录，默认仍2048，没有改变生产模型预算。
...
[六次追问](evidence/minimax-temporal-followup-v1.jsonl)复用轮次109全部四份合法摘要，不重新生成；完整历史两次作为参考。所有回答JSON/schema合法。首候选将取消时刻答为归档时刻2026-03-06 20:45，并回答两者是同一事件；实际取消在3月4日18:15。这与摘要中的“归档作废”合并一致，说明仅检查五个日期值不足以发现时间关系损失。第二候选和两次完整历史均正确区分取消、归档。
...
[结构差异](evidence/minimax-estimator-manifest-diff-v1.json)仅三个golden字段变化：manifest标签、描述及provider投影描述；参数、digest、schema和所有调用上限不变。修改后24项后端定向及共享TypeScript契约测试通过。首次执行编辑脚本因工作目录下相对解释器路径错误未修改文件；修正绝对路径后重新编辑和测试，没有把修改前测试当作验证。

[真实M3核验](evidence/minimax-estimator-manifest-live-v1.jsonl)两次调用提交，首轮实际请求详情与估分；[两次adapter观测](evidence/minimax-estimator-manifest-observations-v1.json)均带上新描述。六章节时长5+8+5+6+4+2为30分钟，人格比较/解释/边界对应具体活动，但today_tasks另有2分钟圈词，未明确是否包含；版面定位仍粗略。采用范围仅为已证实的能力表述一致性修正，不声明整体质量或性能改善。

## 轮次 112：分离事件与记录时刻仍未稳定通过

[17次调用](evidence/minimax-event-time-pairs-v1.jsonl)在原中文样本和新法文样本上分别ABBA比较“保留日期”和追加“事件分离”指令，另做完整历史参考。法文明确6月3日发生取消、6月4日才记录，并设置另一组人物的取消日期未知；不能拿记录时刻补齐未知事件。每份52条消息，450字符摘要、4096输出token，schema与题目在两个实验组相同。本批将schema与任务指令合为一个system消息，两组一致，不能和上轮变化作单一因果比较。
...
新增无内容诊断：严格JSON对象是否等于输入schema、Pydantic缺失/额外字段路径和类型、JSON错误偏移，不保存错误输入或思考正文。[复用五份合法摘要](evidence/minimax-event-time-replay-v1.jsonl)，不重新压缩、保留原题目和4096预算，五次均有终态。
...
[五次相同摘要重测](evidence/minimax-event-time-instance-v1.jsonl)仅在system末尾增加“返回填有答案值的数据实例，不复述JSON Schema”的提醒，4096预算、题目、来源摘要和顺序保持不变。本批5/5严格JSON与schema合法、无schema回显，4/5六项事实全对。与上一批顺序执行而非交错随机比较，不能宣称稳定泛化改善或排除服务波动。
...
新增隔离运行器，每个样本调用原Study探针一次、使用独立数据库；种子未提交时保留已准入receipt与失败类型，继续其他样本，避免只报告成功。按中文—法文—法文—中文运行[四例](evidence/minimax-study-event-isolated-v1.jsonl)，含种子共24次聊天调用，4/4最终提交及读回一致。全部memory_trace的session_id属于本样本种子，没有上一题答案串入；原始种子助手回复仍可影响检索，这项限制保留。
...
[四组独立数据库ABBA](evidence/minimax-study-scope-pairs-v1.jsonl)统一法文来源和教师人格，候选仅替换会话system中的“默认当前章节”句，明确用户要求记录/检索/核对其他资料时应处理。25次聊天调用，4/4最终提交；每次adapter观测恰好一个原句，候选12次均实际替换。不是轮次42的通用任务优先级追加语句，也未改工具授权或生产提示。
...
[七个合成正反例](evidence/minimax-citation-tokenization-v1.json)直接运行当前引用选择器，并仅在诊断中替换为记忆检索的Unicode/中文bigram分词。当前把“请解释一元一次方程”视为整串，漏掉真正相关中文片段；替换分词后召回，但又因共有“这个”将约定取消问题引用到等式说明。两种实现都因英文功能词或法文子串给无关片段引用。
...
[八个种子receipt审计](evidence/minimax-study-memory-write-audit-v1.json)复用轮次118，不增加provider调用。六个种子有write_session_memory且读回一条专门记忆；基线首组与候选首组的第二种子没有该工具，session_memory为空，但聊天Turn本身已提交。因此“已登记/分开存放”等确认文字不能被当作专门记忆效果证据；普通“记下”也不自动判假，因为会话历史确实保存了用户原话。
...
[四个独立Study操作](evidence/minimax-memory-write-pairs-v1.jsonl)按摘要—原文—原文—摘要比较同一短法文记录，共12次聊天调用，全部write_session_memory、read_session_memory和最终提交/读回通过，每例恰好一个rdv_reference。依据[内容与效果标准](evidence/minimax-memory-write-rubric-v1.json)分别检查原文一致性、语义和最终格式，标准在运行中详细审查前写下，不称为预注册。
