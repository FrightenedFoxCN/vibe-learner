...
| [ ] MQ-06 人格与 Markdown 指令 | 教師人格扩大或拒绝记笔记任务、误认第三方、添加讲解/邀请；格式提醒迁移不稳定，Tavern 会编造关系/共同经历。Persona Source Constraint Capsule live 对照保留全部 8 对分母，仅 6/8 strict；虽有 4 项双方共同 major reduction，仍新增 permission/shared-history major，预注册 promotion 失败。 | [范围候选](evidence/minimax-study-scope-pairs-v1.jsonl)、[人格边界](evidence/minimax-persona-task-boundary-review-v1.json)、[Tavern 迁移](evidence/minimax-tavern-grounding-transfer-review-v1.json)、[跨域 shadow](m3-persona-scene-tavern-shadow-2026-09-13.md)、[Capsule 结果](m3-persona-source-capsule-results-2026-09-13.md)、[格式对照](evidence/minimax-study-format-pairs-v1-grade.json) | 正常/冲突指令和不同人物关系的留出对照；人格措辞与任务权限分开审查；下一种 Persona/Tavern 策略需独立预注册，不得重用本轮失败样本；真实浏览器检查列表、围栏、公式及其他所用 Markdown。 |
...
- MQ-04/05：2026-09-13 的[纯文本提示迭代与冻结修复回放](m3-text-iteration-results-2026-09-13.md)已完成本批开发筛选。等 wire specialist→repair 在两份冻结 major 上 0/2 修复且多用 17.0% tokens，已裁剪；这不否定一般多智能体。按用户更正继续迭代 source-minimal 后，v2 在 12 个新 family 上人工 9 pass/3 minor/0 major，v3 在再一批 12 个新 family 上为 9 pass/3 minor/0 major，相应对照 v2 为 8 pass/4 minor/0 major；v3 本批 tokens 低 12.6%，但仍有完整 span 与台账认识状态 minor。实验结束时的决定是只进入独立确认候选、生产默认不变；此后用户于 2026-09-13 明确授权将 v3 用作 Learning Plan 与 Study Chat 的生产默认。该后续采用不追溯改变实验决定，也不关闭独立质量门：证据仍仅为开发合成样本及内部人工语义审查，没有独立学习效果认证。相似开发素材停止，multimodal 继续暂缓，小模型不与提示/架构同时变更。
