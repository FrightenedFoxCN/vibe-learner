...
The Tavern is a separate interaction domain for conversations with saved personas and for user-directed interaction between personas. It reuses persona and scene snapshots, but it does not inherit the textbook, plan, citation, or Study Unit requirements of a Study Session. Its validation uses the repository-wide harness trace contract described in [the Harness guide](harness/README.md) and the [Tavern workflow page](harness/tavern.md); the harness is not Tavern-only.
...
Fresh SQLite and PostgreSQL schemas enforce the retry self-foreign-key. The local SQLite compatibility migration rebuilds earlier `tavern_runs` tables instead of only adding a column, preserves existing run-step rows, restores the self-FK, and finishes with `PRAGMA foreign_key_check`. Managed database transactions collect Tavern room/run/message identities at the connection DML boundary, retain them across ORM flushes, and validate affected Message/Run/Step soft references before outer commit. Incoming references and deleted identities remain in scope; unrelated Settings writes issue no Tavern queries. The full invariant scanner is an explicit maintenance audit. Managed writes use structured SQLAlchemy DML with literal scope identities; opaque mutation SQL, expression scope identities, nested DML and INSERT-from-SELECT are rejected rather than silently bypassing validation. Engine-level migration/maintenance operations remain outside this managed-session boundary and require explicit audit. SQLite savepoints establish an outer transaction before SAVEPOINT so RELEASE cannot persist invalid data before outer validation. The rules use backend-independent SQLAlchemy queries. The same 22 transaction/scope contracts passed against temporary SQLite and a local PostgreSQL 17 instance; the additional legacy corruption-audit fixture remains SQLite-specific.
...
Static/state tests, populated real-backend wire acceptance, keyboard composition fencing, authoritative recovery/roster derivation, structured mutation-error and provider-call fencing, keyboard focus return, and an earlier 390px no-overflow inspection have passed. A non-implementer live matrix covered direct/facilitated/partial/resume/cancel state, recovery lineage, refresh, retry badges, 502 read-back, and failure cases. Tavern navigation, action buttons, Setup inputs/selects, older-message control, and provider details now have 44px static touch-target contracts; the v0.2.1 acceptance environment could not expose a dedicated 390×844 browser viewport, so a fresh device-level visual/touch measurement is not claimed. `REL-DESKTOP-001` remains open for the no-Persona/no-Room viewport, device-level IME, full focus order, and that 390×844 device-level visual/touch measurement; the static 44px contract is not a substitute. The optional `TAVERN_TEST_API_URL` decoder fixture is skipped unless a populated backend with messages and a terminal run is explicitly supplied.

The versioned `tavern-copy-v1` contract in `apps/web/lib/tavern-copy.ts` owns run/step labels and recovery/error copy. A blocked actor is explicitly not executed because an earlier actor did not complete; historical labels retain their previous-round scope. Unknown error codes use application-authored fallback text, with raw evidence retained in Debug Overlay. Cancellation copy distinguishes rejecting a result from stopping upstream execution. `tests/tavern-copy.test.ts` gates the state vocabulary, actionable stale/archive/retry messages and raw-code isolation; browser evidence is recorded separately.
...
- Transcript pressure is recovered only by sorting committed messages by `(sequence, id)` and removing the oldest first. Scene, Persona, cast, or final-total overflow raises a stable `tavern_prompt_*` error before the provider boundary. When an output reaches Tavern semantic validation, its successful preflight report records budget version, original/final bytes, token estimate, and removed-message count; rejected preflight records the failed partition plus numeric actual/limit. A later decode failure keeps its decode phase/error without claiming a successful budget check. Production actor steps resolve an authorized protected snapshot and persist v3 attempts, checks, and terminal evidence; historical v1/v2 shapes remain content-free compatibility records. Real-tokenizer/provider p50/p95 and regression thresholds are tracked by `PERF-TAV-LIVE-001` in the root TODO.
...
Actor generation requests strict `TavernActorReply` JSON in which every declared property is required and exposes only model-owned text/performance/target proposals. Before prompt preflight or provider work, `tavern-actor-workflow-adapter-v2` resolves exactly one operation-authorized `TavernActorProtectedSnapshot` v2 and strictly decodes its canonical bytes. The v2 snapshot carries the Run identity/context digest, server-owned schedule and step, complete actor Participant, Room/roster, authoritative transcript sequence, exact reply anchor, derived required target, user message, and guidance. Runtime validation compares that decoded projection with an independently constructed projection from the persisted Run, claimed Speaker Step, and current repository read; it also binds the actor prompt hash and Room subject reference. A canonical and internally self-consistent replacement Persona, Scene/policy, roster/order, schedule, stale transcript, or reply anchor therefore fails before the provider. Prompt preflight receives an isolated copy of the protected projection and may only delete an oldest transcript prefix; an in-place protected-input mutation or injected, modified, reordered validation message fails closed. A successfully validated output appends a content-free `authorized_snapshot_binding` check to its trace; a failure before or during decode/validation retains its precise phase/error evidence and does not claim that successful check. Protected Persona, Scene, transcript, user-message and guidance content stays only in the artifact. This binding is authorization/integrity evidence, not semantic-fidelity proof, and it does not expand the `primary_output_only` commit policy. The semantic harness then checks speaker attribution, target membership, the required prior-speaker target, length, prompt-material leakage across every persisted display field, and non-empty output. `tavern-speaker-identity-v2` centrally compiles every other participant's display name, Persona ID, and collision-reviewable ID alias (for example `persona-b` → `b`); it allows ordinary third-person mentions but identifies quoted/Markdown/script speaker labels, attributed quotations, and other-role inner activity across Unicode colon variants. Identity comparison uses whole-string compatibility/caseless normalization plus a reviewed set of invisible default-ignorable format controls, while deterministic repair maps the match back to the original text before truncation and then re-scans every model-owned display/performance field; an empty or remaining violation fails closed. Transport/schema recovery is merged into the persisted trace; exhausted decode and provider failures also produce failed traces. Failed output is not stored as a persona message. Recent-run history remains a bounded debug window (default 50). `GET /tavern/rooms/{room_id}/run-recovery` independently loads complete room lineage, validates a single linear child chain per root, and returns a bounded set of root summaries whose `leaf_run` determines `active`, `recoverable`, `recovered`, `completed`, or `canceled` state.
