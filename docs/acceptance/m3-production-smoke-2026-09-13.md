...
- **观察（已修复）**：分功能验证曾让所有“拉取/验证/能力刷新”控件同时进入 loading。现在只标记共享同一有效 endpoint 与 credential 的冲突 scope；不同连接仍可独立操作，共享探针失败也会释放全部受影响 scope，不再留下永久“验证中”。
- **观察（已修复）**：上游 `/models` 成功列出 8 个模型，但图像输入与联网能力字段均无法识别。UI 继续诚实显示“未知”，并明确说明这既不代表支持也不代表不支持，手工开关由用户决定，错误开启可能导致请求失败。
...
- **UI 问题（已修复）**：Plan Workspace 在 1440px production 视口采用设置/结果双栏，主结果列显著宽于设置列，长计划文本保持受控换行和可选择复制。
...
- **可访问性/自动化问题（已修复）**：教师人格、计划场景、创建方式、教材文件和学习目标现在都具有一一对应的 `<label for>` / `id`，production Chromium 同时按 DOM 关联和 accessible name 验证。
