...
The typed database-effect slice now covers memory, affinity, follow-up scheduling/completion/cancellation, projected-document/image set/focus/overlay/clear, and plan confirmations. Their strict discriminated proposal union contains bounded effect content but no committed record, operation, batch, effect, Session revision, verdict, or timestamp identity. A single in-process collector assigns a global append-only slot plus `(operation_id, slot)`-derived effect identity across every proposal kind. Memory, affinity, follow-up, and projection reads apply prepared slots over the authoritative Session so repeated writes and dependent tool calls are visible in model order without an early database write.

The final transaction first validates the full operation/batch/slot/adapter/contract/target set—including authoritative Plan schedule and learner-attachment projection membership—then applies every Session database effect in slot order with the validated Turn, Session revision, operation result, and a server-only `StudyChatCommittedEffectBatchV1`. Read-back binds the projections to the committed Session snapshot; public receipts do not serialize this internal evidence. A validation, adapter, response-build, CAS, or receipt failure rolls back every slot. The prepared batch is still in-process state—not a durable prepare journal, public effect receipt, v3 Harness stage, or provider exactly-once proof. Scene JSON, attachments/files, and generated/provider artifacts still cross separate boundaries; generated-image completion is explicitly reported as external and uncommitted. The adapter vocabulary and shared golden registry establish schema ownership for later workflow adoption; they do not close `SCH-HRN-EFFECT-001`, `STUDY-EFFECT-COMMIT-001`, `HRN-STUDY-001`, or the full `HRN-WEB-STUDY-DEC-001` gate.
...
| Study chat | reply recovery, tool trace, citations, Study Session revision CAS/sequenced Turn append, durable request admission, terminal result read-back, strict operation-receipt decode, mixed transactional Session database effects | remaining Scene/file/provider adapters, durable prepare/outbox evidence, complete nested response decoder, protected replay and v3 evidence |
