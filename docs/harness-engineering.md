...
Protected artifact policy is similarly explicit. The completed authorization contract chooses a single-user local-installation principal resolved by the server, not a bearer token. Grants are operation-scoped and bind one principal to exact artifact type/ID/contract/permission scopes plus issue, expiry, and revocation time. Audit results bind both requested and granted operation/principal identity. Missing principal, cross-principal or cross-operation rebinding, expired/revoked grants, and scope mismatch fail closed without putting secrets into evidence. The durable resolver/storage layer remains unfinished.
