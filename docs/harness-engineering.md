...
The first typed-effect slice now covers only the Study tools `update_learning_plan` and `update_learning_plan_progress`. Their strict proposal contains model-suggestible plan content but no confirmation, operation, batch, effect, Session, revision, or timestamp identity. The application assigns `(operation_id, slot)`-derived effect identity, validates the registered database adapter and Session target, and delays `SessionPlanConfirmationRecord` creation until the final reply has passed validation. The confirmation, Turn, Session revision, and operation receipt then commit in one database transaction, so an invalid final reply or fenced final commit leaves no orphan plan confirmation.

This prepared batch is in-process state, not a durable effect journal, public effect receipt, v3 Harness stage, or provider exactly-once proof. Memory, affinity, follow-ups, Scene, projections/overlays, attachments/files, generated/provider artifacts, and other Study writes still cross separate boundaries and may occur before final reply validation. The adapter vocabulary and shared golden registry establish schema ownership for later workflow adoption; they do not close `SCH-HRN-EFFECT-001`, `STUDY-EFFECT-COMMIT-001`, `HRN-STUDY-001`, or the full `HRN-WEB-STUDY-DEC-001` gate.
...
| Study chat | reply recovery, tool trace, citations, Study Session revision CAS/sequenced Turn append, durable request admission, terminal result read-back, strict operation-receipt decode, typed transactional plan-confirmation proposals | remaining memory/affinity/follow-up/Scene/file/provider effect adapters, durable effect evidence, complete nested response decoder, protected replay and v3 evidence |
