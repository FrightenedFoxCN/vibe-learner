...
The typed database-effect slice now covers `write_session_memory`, `update_affinity_state`, `update_learning_plan`, and `update_learning_plan_progress`. Their strict discriminated proposal union contains model-suggestible content but no record, operation, batch, effect, Session, revision, verdict, or timestamp identity. A single in-process collector assigns a global append-only slot plus `(operation_id, slot)`-derived effect identity across all three proposal kinds. Memory and affinity reads apply the prepared slots over the authoritative Session so repeated same-key writes and affinity deltas are visible in model order without an early database write.

The final transaction first validates the full operation/batch/slot/adapter/contract/target set, then applies memory upserts, affinity deltas, and plan confirmations in slot order with the validated Turn, Session revision, operation result, and a server-only `StudyChatCommittedEffectBatchV1`. Read-back binds the projections to the committed Session snapshot; public receipts do not serialize this internal evidence. A validation, adapter, response-build, CAS, or receipt failure rolls back every slot. The prepared batch is still in-process state—not a durable prepare journal, public effect receipt, v3 Harness stage, or provider exactly-once proof. Follow-ups, Scene, projections/overlays, attachments/files, generated/provider artifacts, and other Study writes still cross separate boundaries. The adapter vocabulary and shared golden registry establish schema ownership for later workflow adoption; they do not close `SCH-HRN-EFFECT-001`, `STUDY-EFFECT-COMMIT-001`, `HRN-STUDY-001`, or the full `HRN-WEB-STUDY-DEC-001` gate.
...
| Study chat | reply recovery, tool trace, citations, Study Session revision CAS/sequenced Turn append, durable request admission, terminal result read-back, strict operation-receipt decode, mixed transactional memory/affinity/plan-confirmation effects | remaining follow-up/Scene/projection/file/provider effect adapters, durable prepare/outbox evidence, complete nested response decoder, protected replay and v3 evidence |
