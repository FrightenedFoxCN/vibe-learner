...
- `HarnessTraceV3` preserves the v2 lifecycle fields while requiring `HarnessContextEnvelopeV3`: a closed workflow/stage pair, registered component set, typed subject/snapshot references, service-owned operation ID, named input/context/digest contracts, and a trace-visible context digest that validators recompute. V3 is the production contract for adopted workflows; it does not rewrite historical v2 evidence.

Unknown explicit trace-schema versions are rejected. Missing `trace_schema_version` means legacy v1; old records are not assigned invented v2/v3 identities, timestamps, attempts, or commit claims. New workflow integrations emit v3. Tavern actor generation and Study Chat now emit v3 from their production paths, while historical v1/v2 rows remain read-only compatibility evidence.
...
- `tavern_message` is an append-only committed resource. Its actor-reply policy permits exactly one sequence point and binds room/message/sequence through a versioned committed projection. Message IDs remain invalid as v3 context freshness evidence; transcript context is supplied through an authorized protected room snapshot.
- `study_session` has authoritative-revision context and commit evidence for the Study Chat Session/Turn primary-output projection.
...
V3 operation claims also fail closed through `HARNESS_OPERATION_COMMIT_POLICIES`. The full key is workflow, stage, output-contract name/version, and committed-projection contract name/version; a generic resource capability cannot authorize a business operation. The Tavern `actor_reply` policy permits exactly one attempted/committed `tavern_message`, a single-sequence proof, and `TavernPersonaMessageCommittedProjection/tavern-persona-message-committed-projection-v1`. The Study `study_chat_reply` policy permits one revisioned `study_session` and binds `StudySessionTurnCommittedProjection/study-chat-turn-committed-projection-v1`. Both use `primary_output_only`: Tavern proves one persona Message rather than all Room/Run/Step changes, and Study proves the Session/Turn rather than every Scene, file, or provider effect. Unsupported operations remain fail closed.
...
`HarnessEffectTerminalEvidenceV1` records exactly one `committed`, `not_committed`, or `uncertain` result with provider correlation, read-back capability/outcome, exact committed-projection digest or compensation evidence, and ordered UTC timestamps. A provider adapter with unsupported read-back cannot produce `committed` evidence. The Study database-effect collector prepares through this journal on the production route, and its terminal read-back records share the final Session/Scene/Turn/operation/trace database transaction. Attachment cleanup and provider uncertainty retain their separate truthful boundaries; the Session/Turn operation policy does not claim provider exactly-once or complete Scene/file effect coverage.
...
| Study chat | production v3 protected context/runtime, strict complete nested reply decode, deterministic held-out eval, independently revalidated safe-retry, citations, Session/Scene CAS commit, durable request admission, operation-owned attachment staging/read-back, truthful provider uncertainty, mixed typed effects, durable database-effect preparation and atomic terminal read-back | optional live-wire decoder samples, per-provider-call journal/read-back capability, public effect-evidence policy |
| Tavern | production v3 protected context/runtime, deterministic identity eval, normalized room/run/step schema, low-trust persona compiler, strict actor/browser decode, authoritative retry-chain projection, per-actor commit, partial state, scoped child retry, leased resume/cancel fencing, provider-preflight prompt budgets, invariant scanner, independent recovery browser/wire matrix | no-Persona/no-Room viewport, device-level IME, full focus order and 390×844 visual/touch measurement; real-tokenizer/provider thresholds |
...
The matrix records incremental adoption across the repository. Tavern actor generation and Study Chat have passed the production v3 effect, failure, replay, and eval gates. Parsing, planning, persona/scene generation, and frontend decoding remain open until their own boundaries pass the same rubric.
...
Production Tavern actor records carry v3 protected-context, attempt/check, and Message commit evidence. The frontend strictly decodes v3 while preserving historical v1/v2 compatibility. Its `primary_output_only` policy still does not prove every Room, Run, or Step effect in the actor transaction.
