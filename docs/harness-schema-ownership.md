...
`StudyPlanConfirmationEffectProposalV1` is the first concrete typed database proposal. It owns only the bounded plan action, title or schedule/status content, and note. `HarnessPreparedEffectV1` adds the application-owned operation, batch, effect, slot, adapter, proposal-contract, and Study Session/Learning Plan target references; committing it derives the confirmation identity and timestamp. The current collector is in-process and its committed projection is internal read-back material, not a durable effect receipt or registered v3 operation claim. Only the two plan-confirmation tools share the final Turn/Session/operation transaction; remaining Study effects, per-effect terminal evidence, file/provider compensation, protected artifacts, v3 context/trace, strict nested API decode, and eval fixtures remain owned by `SCH-HRN-EFFECT-001`, `STUDY-EFFECT-COMMIT-001`, and `HRN-STUDY-001`.
