...
The operation state is not effect evidence. `admitted` / `running` / `uncertain` never authorize blind replay; `not_committed` authorizes retry only with durable proof that execution never began. A committed final Turn does not imply that earlier model tools, file preparation, provider requests, or external effects share its transaction. Typed proposals, application-owned `(operation, slot)` effect identity, prepare/commit/compensate/read-back adapters, and `not_committed` / `committed` / `uncertain` semantics per effect remain owned by `SCH-HRN-EFFECT-001` and `STUDY-EFFECT-COMMIT-001`. Only after those boundaries plus protected artifacts, v3 context/trace, strict nested API decode, and eval fixtures land may `HRN-STUDY-001` describe Study Chat as harnessed.
