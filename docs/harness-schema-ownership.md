...
| Study Chat | stable client request ID, expected Session revision, learner input/attachment manifest, bounded history, persona/scene/document snapshots, allowed toolset | reply, citations, Character Event content and typed effect proposals | operation/turn/event IDs, line segment IDs, memory/affinity/follow-up/scene/projected-document/plan-confirmation effects, revision and timestamps | operation receipt; committed receipt contains chat exchange and refreshed Session | adopted in Wave 3 |
| Tavern | room revision, trigger, roster snapshots, bounded transcript and policy | `TavernActorReply` display/performance/target proposal | speaker, room/run/step/message IDs, schedule, reply anchor, sequence, lineage, revision and terminal state | Tavern room/run/message responses | actor generation adopted in Wave 3 |
...
Tavern Room and Study Session have authoritative-revision context/commit shape support. A Room revision covers control metadata plus run admission rather than transcript history; a Session revision covers its authoritative aggregate and contiguous Turn sequence. Tavern Message has single-sequence append commit support but is not a valid context subject because its ID alone is not room-scoped freshness proof. Production Tavern and Study context therefore use authorized protected snapshots instead of treating embedded child IDs as freshness evidence. These generic shapes do not by themselves prove operation semantics: the registered operation policy and committed projection must also bind the exact workflow/stage/output contract and permitted resource set. Document, page, debug, Study Unit, plan/trace, Persona, Scene, Tavern Run, and frontend request remain unsupported until their domain adoption commits add truthful evidence.

Study Chat registers `study_session` freshness and commit semantics through `StudySessionTurnCommittedProjection/study-chat-turn-committed-projection-v1`. Authoritative read-back binds the admitted operation, expected/committed Session revision, committed Turn ID/sequence/digest, and exact projection digest. Its `primary_output_only` scope proves the Session/Turn output; it does not attach the Session revision to memory, attachment, Scene, or provider effects as though they were the same resource.
...
redacted or content-free. This boundary supports the adopted Study Chat
workflow but does not make Planning v3-adopted.
...
The Tavern policy adopts `TavernActorReply/tavern-actor-reply-v2` and exactly one persona Message projection. `TavernPersonaMessageCommittedProjectionV1` contains the complete committed record including content; a content-free `TavernPersonaMessageCommitBindingV1` carries identity plus its canonical digest. Server-only `TavernPersonaMessageCommitMetadataV1` persists operation/effect-batch identity in the same transaction as the Message and is deliberately absent from the API/OpenAPI schema. A repository method reads Message/Run/Step/Participant/reply-anchor from one database snapshot, and the sidecar rejects cross-room, cross-run, cross-step, reply-anchor, schedule, Persona snapshot/hash, request, operation/effect-batch, sequence, and digest mismatches even if an attacker recomputes a self-consistent caller payload. The policy is explicitly `primary_output_only`, not complete transaction-effect evidence: Step/Run state and Room sequence watermark still lack committed resource projections. Begin-run and Room create/delete remain unsupported. Production resolves protected context and persists v3 attempts/checks and terminal evidence; historical v1/v2 traces remain readable without reinterpretation.

The Study policy adopts `StudyChatReply/study-chat-reply-trace-v1` and one revisioned Session/Turn projection. Its authoritative read-back includes the operation binding, expected and committed Session revisions, Turn ID/sequence, complete Turn digest, and projection digest. Session/Turn/effect journal/operation/terminal trace commit together, while the policy stays `primary_output_only` and makes no full-transaction claim for Scene rows, attachment files, or provider work. The server-only Harness trace and effect batch are omitted from the public Study receipt.
...
| Persona, Scene, Frontend Decode | existing broad generation/decode boundaries | domain components | component versions deliberately unregistered |
| Study Chat / `study_chat_reply` | one admitted request, complete nested reply, effects, and Session/Turn projection | prompt, toolset, strict v3 adapter | production v3 trace/protected replay/eval adopted |
| Tavern / `actor_reply` | one scheduled persona proposal and its validation | prompt, persona compiler, scheduler | production v3 trace/protected replay/eval adopted |
