...
Study memory retrieval selects bounded original-text neighborhoods using the actual query before embedding/ranking. Learner excerpts remain capped at 800 characters including normalized-text position labels; assistant excerpts remain capped at 160. Selection preserves source order and marks partial coverage. It creates no model-generated memory record, changes no ownership boundary, and does not prove all relevant history fits the excerpt.

Study grounding now registers `study-grounding-v1`: a citation-only normalized tokenizer and explicit `/remember-verbatim` source binding. New protected Study snapshots use v2 with the original learner message, resolved after operation-scoped authorization; v1 remains decode-only compatibility. The adopted boundary is summarized in [model runtime quality](model-runtime-quality.md); experimental acceptance remains on the local evidence archive branch. Public DTOs and memory effect v1 are unchanged.
