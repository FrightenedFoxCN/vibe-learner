...
- 轮次总览
  - 展示流式阶段状态、轮次数与工具调用数。
- Round 明细卡
  - 每轮显示状态、finish reason、耗时、错误与工具调用标签。
  - 模型 recovery 次数和说明目前只在 Debug Overlay/计划 trace 中查看，主计划界面不承诺展示。
...
- 对话失败或结果未确认提示
- 超时、断线、解码失败或 `admitted` / `running` / `uncertain` 时只能查询原请求结果，不能重新发送
- 只有服务端明确返回 `not_committed` 且 `safe_to_retry=true` 时才允许重新发送；明确未 admission 的 4xx 先刷新会话状态并保留草稿
...
- 附件按钮
  - 每次最多 4 个文件。
  - 单个文件最大 12 MiB，全部附件合计最大 24 MiB。
  - 刷新页面无法恢复浏览器 `File` 对象；提交前请确认附件仍在草稿列表。
- 发送按钮
...
- 可从已保存的 Scene Library 条目中附带场景快照；未保存的 Scene Setup 草稿不会出现在这里。源人格或场景后续修改不会悄悄改写既有房间快照。
- 若没有可选人格，先前往人格色谱创建人格。
...
- 当前运行以服务端 speaker step 为准；正在生成、等待发言与终态不会被本地预测覆盖。没有运行时，历史结果明确标为“上一轮”。
- 窄屏无房间时先显示最近房间与创建表单；已有房间时先显示 Composer。点击“新建酒馆”会定位并聚焦标题。
...
- 检查后端服务是否在线。
- 检查 PDF 是否有效。
- 在计划生成页查看计划轮次区，确认是否有错误、finish reason 与工具调用；恢复细节请打开 Debug Overlay 查看计划 trace。

### 5.2 对话无法继续

- 在章节对话页检查是否已创建会话。
- 检查当前章节/子章节是否有效。
- 若界面要求“查询本次请求结果”，先查询原 operation，不要重发；仅在明确可重试时再发送，并到统一设置确认对话模型配置。
