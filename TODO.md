...
- [ ] `SCH-HRN-REV-001` 建立 machine-readable resource evidence policy；验收：每个 `HarnessResourceType` 分别声明 context revision 要求与 commit evidence 能力，validator 对缺失/伪造 revision、sequence 与不支持的 committed resource fail closed，Python/TypeScript registry 完整对齐。不得以常量 `revision=0` 伪造 freshness，也不得把父聚合 revision 冒充嵌入资源自身 revision。
